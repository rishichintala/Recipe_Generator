...
//     const isValid =
//       /^[a-zA-Z\s-]+$/.test(trimmed) && trimmed.length >= 2 && trimmed.length <= 30;
  
//     if (!isValid) {
//       setMsg("❌ Invalid input. Please enter real ingredients only.");
//       clearMsg();
//       return;
//     }
...
//       setMsg("Please enter at least 2 ingredients.");
//       clearMsg();
//       return;
//     }
...
//       <div
//         key={`${section}-${idx}`}
//         className="bg-white rounded-xl p-4 shadow hover:shadow-md transition-shadow flex flex-col justify-between h-full"
//       >
//         <div>
...
//           className="mt-4 w-full bg-green-100 hover:bg-green-200 text-green-700 py-2 rounded flex justify-center gap-2 items-center"
//         >
...
//               <X size={20} />
//             </button>
//             <h3 className="text-xl font-semibold text-green-700 mb-3">{modalRecipe.name}</h3>
//             <ol className="list-decimal list-inside space-y-2 text-sm text-gray-800">
//               {modalRecipe.instructions.map((step, i) => (
//                 <li key={i}>{step}</li>
//               ))}
//             </ol>
//           </div>
...
function App() {
  ...
  const generate = async () => {
    ...
    if (ingredients.length <= 2) {
      ...
      // ⚠️  DO NOT call clearMsg() here – leave it visible
    } else {
      setMsg("");      // no notice needed
    }
